...
return L.view.extend({
	render: function() {
		...
		m = new L.form.Map('warpengine', _('WarpEngine Ultra Edition'),
			_('Full-stack network acceleration solution designed for OpenWrt 24.10 (nftables/fw4).'));
			...
			return L.resolveDefault(callStatus(), {}).then(function(res) {
				...
				return E('div', { 'class': 'cbi-section-node' }, [
					E('div', { 'style': 'padding: 15px; line-height: 2.2;' }, [
						_('Kernel Version'), ': ', E('strong', [ res.kernel || 'unknown' ]),
						' | ', _('Active Flows'), ': ', E('strong', { 'style': 'color:blue' }, [ res.flow_count || 0 ]),
						' | ', _('Hardware PPE Status'), ': ', renderStatus(res.hw_status),
						E('br'),
		...
		s.anonymous = true;
		o = s.option(L.form.Flag, 'enabled', _('总控开关'));
		o.rmempty = false;
		...
		o.description = _('Optimizes multi-core data packet processing efficiency. Requires multi-core CPU.');
		o.depends('enabled', '1');
